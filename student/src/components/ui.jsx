...
export function Modal({ open, onClose, title, description, children, labelledBy = "modal-title" }) {
  ...
  useEffect(() => {
    ...
    const shouldInertMain = main && !main.contains(panelRef.current);
    if (shouldInertMain) {
  ...
  return (
    <div
      className="modal-root fixed inset-0 z-50 grid place-items-center p-4"
      role="dialog"
      aria-modal="true"
    ...
    >
      <div
        ...
        aria-hidden="true"
      />
      <div ref={panelRef} tabIndex={-1} className="modal-panel relative w-full max-w-md rounded-lg border border-line bg-surface p-6 shadow-pop">
        {onClose && (
