...
export default function MyApplications() {
  ...
  return (
    <Page width="5xl">
      ...
      <div className="mt-6">
        {loading ? <SkeletonList rows={3} /> : applications.length === 0 ? <EmptyState title="No applications yet" description="Once you apply to an event, its full progress will be tracked here." action={<Button to="/events">Browse ongoing events</Button>} /> : visible.length === 0 ? <EmptyState title="No matching applications" description="Try another event, club, team, or status." action={<Button variant="secondary" onClick={() => setSearch("")}>Clear search</Button>} /> : <div className="stagger space-y-4">{visible.map(({ _id, role, registrationId: application, history, reason, verticalTitle }) => {
          const event = application.eventId;
