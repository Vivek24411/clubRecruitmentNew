...
import { EventCard, RecruitingClubCard, SessionCard } from '@/components/catalogue';
...
import { eventApplicationsOpen, sessionStart, sessionTiming } from '@/lib/date';
...
export default function HomeScreen() {
  ...
  const recruitingClubs = query.data?.recruitingClubs ?? [
    ...new Map((query.data?.events || [])
      .filter((event) => event.eventType === 'recruitment' && eventApplicationsOpen(event) && event.clubId)
      .map((event) => [event.clubId!._id, event.clubId!] as const)).values(),
  ];
  return (
    <Screen refreshing={query.refreshing} onRefresh={query.refresh} contentStyle={styles.content}>
      ...
      {query.loading ? <LoadingState /> : query.error ? <ErrorState message={query.error} onRetry={query.reload} /> : (
        <>
          {recruitingClubs.length ? <View style={styles.section}>
            <SectionHeader eyebrow="RECRUITING NOW" title="Clubs with open applications" action={<Pressable onPress={() => router.push('/(tabs)/clubs')}><Text style={styles.link}>All clubs</Text></Pressable>} />
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.recruitingRow}>
              {recruitingClubs.map((club) => <RecruitingClubCard key={club._id} club={club} />)}
            </ScrollView>
          </View> : null}
...
const styles = StyleSheet.create({
  ...
  section: { gap: spacing.lg }, recruitingRow: { gap: spacing.md, paddingRight: spacing.lg }, link: { color: palette.accentDark, fontFamily: typography.semibold, fontSize: 13 },
});
